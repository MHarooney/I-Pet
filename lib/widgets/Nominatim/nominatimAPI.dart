...
class NominatimAPI {
  ...
  NominatimAPI(
      {this.host = "https://nominatim.openstreetmap.org",
  ...
  Future<List<Address>> resolve({String query, bool verbose = false}) async {
    ...
    if (!host.startsWith("https://") && !allowInsecure) {
      print(
          "WARNING: Using http hosts is not allowed (Override this using `allowInsecure: true`)");
      return null;
    }

    /// Make sure we are allowed to do the request
    if (DateTime.now().millisecondsSinceEpoch -
            _lastRequest.millisecondsSinceEpoch >
      ...
      String uri =
          "$host/search/$query?format=jsonv2&addressdetails=1&accept-language=en";
      ...
      print(
          "WARNING: Cooldown, your latest request was less than the cooldown duration. (${DateTime.now().millisecondsSinceEpoch - _lastRequest.millisecondsSinceEpoch}/${cooldown.inMilliseconds}ms)");
  ...
  /// Resolve a [query] to the [host] specified in this class
  ///
  /// Returns List of cities/countries if successful (If not returns [null])
  ///
  /// [query] defines the address we are looking for
  /// [verbose] is for outputting debug data
  Future<List<Address>> getCitySuggestions(
      {String query, bool verbose = false}) async {
    /// Check if we are running this request securely
    if (!host.startsWith("https://") && !allowInsecure) {
      print(
          "WARNING: Using http hosts is not allowed (Override this using `allowInsecure: true`)");
    ...
    if (DateTime.now().millisecondsSinceEpoch -
      ...
      List<Address> aList = new List<Address>();
      String uri =
          "$host/search/?city=$query&format=jsonv2&addressdetails=1&accept-language=en";
